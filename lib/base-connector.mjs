...
export class BaseConnector extends EventEmitter {
  ...
  _scheduleReconnect() {
    ...
    if (this._reconnectTimer) return; // prevent duplicate scheduling
  ...
  /** Clear local book. Subclass must override. */
  _resetBook() {
    throw new Error(`${this.market}: _resetBook() must be overridden`);
  }

  /** Clear local book and snapshot tracking. */
  _resetBook() {
