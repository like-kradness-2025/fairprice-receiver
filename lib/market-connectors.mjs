...
export class BinancePerpBtcusdcConnector extends BinancePerpConnector {
   ...
   * has arrived during the REST fetch. A bridging diff is still required
   * (via _syncBook) before entering running so the pu chain is intact.
   */
  _validateSync(snapshot) {
    if (this._ringBuf.length === 0) {
      return true;
  ...
  /**
   * Override sync to wait longer for a bridging diff before entering running.
   * Low-volume markets need generous retry with wait to accumulate diffs.
   * Clears the ring buffer between retries so stale non-bridging diffs don't
   * block progress.
   */
  async _syncBook() {
    this._setState('syncing');
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        ...
        if (valid) {
          this._applyRingBuf(snapshot);
          if (!this._ringBufApplied) {
            // No bridging diff yet — clear stale buffer so fresh diffs
            // can accumulate for the next snapshot attempt.
            this._ringBuf = [];
            this._ringBufPos = 0;
            continue;
          }
